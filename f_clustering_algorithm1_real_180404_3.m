function [result1 result2] = f_clustering_algorithm1_real_180404_3(CM,n_m,n_m_g)
                   ...
                   while l_complete ~= l
                       ...
                       for i = 1 : C_n
                           ...
                           for j = 1 : length(Cluster_result{i})
                               test_c{i}(1,:) = test_c{i}(1,:)+CM_t(Cluster_result{i}(j),:);
                           end
...
%                    
                       ...
                       if h < 3
                        cl_c_p_p = cl_c_p_m{1};
                        Cluster_result_s{C_n} =  Cluster_result_sp_m{1}; 
...
%         cl_c_p2
    ...
    for i = 1: length(Cluster_result)
        ...
        for j = 1:length(Cluster_result{i})
            Cluster_result_n{C_n}{i}(j) = n_m_t(Cluster_result{i}(j));
...
if length(outlier_i) > 0

result1{length(result1)+1} = outlier_i;

result2{length(result2)+1} = outlier_g;
end
